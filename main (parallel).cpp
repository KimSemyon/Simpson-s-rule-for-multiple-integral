...
double epsilon = 1e-15;// çíà÷åíèå áëèçêîå ê íóëþ
...
double SerialSimpsonsFunc(double Segments[], double H[], int n, int N, int _param, double* X, std::vector <double> &F)
{
	...
	double h = H[param];
	int count_i = 0;
	int count_res = 2 * n*deep_recursion; // ñ÷åò÷èê äëÿ "external" çàïîëíåíèÿ çíà÷åíèé ôóíêöèé
	for (double i = a; count_i<2*n; i += h, count_i++)
	{
		if (param < (N - 1)) { 
			X[param] = i; 
			param++;
			F[count_res] = SerialSimpsonsFunc(Segments, H, n, N, param, X, F); //Çàïîëíåíèå F 2n ïîçèöèé 
			count_res++;
			param--; 
		} //
		else {
...
double ParallelSimpsonsFunc(ThreadPool &p, double Segments[], double H[], int n, int N, int _param, double* X, std::vector <double> &F, int nthreads)
{
	...
	for (double i = a, count_i = 0; count_i < 2 * n; i += h, count_i++)
	{
		if (param < (N - 1)) {
			X[param] = i;
			param++;
			F[count_res] = ParallelSimpsonsFunc(p, Segments, H, n, N, param, X, F, nthreads); //Çàïîëíåíèå F 2n ïîçèöèé 
			count_res++;
			param--;
		} //
		else {
...
int main() {
	...
	int N; // ðàçìåðíîñòü èíòåãðàëà
	int n; // êîëè÷åñòâî òî÷åê íà îòðåçêå /2
		
	cout << "Enter the dimension of the integral: ";
	cin >> N;

	double *Segments = new double[2 * N]; // Âûäåëåíèå ïàìÿòè äëÿ ìàññèâà ïðåäåëîâ èíòåãðèðîâàíèÿ
	...
	for (int i = 0; i < 2*N; i++) { // Çàïîëíåíèå ìàññèâà îòðåçêîâ 
		cin >> Segments[i];
	}
	...
	double *H = new double[N]; //Âû÷èñëèëè çíà÷åíèÿ øàãîâ
	for (int i = 0; i < N; i++){
...
////////////////////////////////////////////////////////////////////////////////////SERIAL:	
	...
	*/
	...
	//
	//if (fabs(REAL_result - result) < epsilon) { printf("Ðåøåíèå ïîëó÷åíî"); break;}
	//
